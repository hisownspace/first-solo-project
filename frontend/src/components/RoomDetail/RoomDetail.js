...
function RoomDetail() {
  ...
  const resetTransition = e => {
    e.target.style.setProperty("--transition", "opacity 0.35s ease-in");
  }

  const handleButtonGradient = e => {
    const btnWidth = e.target.getBoundingClientRect();
    const x = e.clientX - btnWidth.left;
    e.target.style.setProperty("--x", x + "px");
    e.target.style.setProperty("--x1", (x - 120) + "px");
    e.target.style.setProperty("--x2", (x + 120) + "px");
    e.target.style.setProperty("--rgb", "255, 65, 65");
  }

  const resetButtonGradient = e => {
    console.log(e.target)
    // e.target.style.setProperty("--x", "220px");
    // e.target.style.setProperty("--x1", "100px");
    // e.target.style.setProperty("--x2", "340px");
    // e.target.style.setProperty("--rgb", "260, 38, 75");
    e.target.style.setProperty("--transition", "opacity 2s ease-in");
  }
            ...
            <div className="reservation-box">
              ...
              <form onSubmit={submitReservation} className="reservation-form">
                ...
                <button
                  ...
                  ref={resBtn}
                  onMouseOver={resetTransition}
                  onMouseMove={handleButtonGradient}
                  onMouseLeave= {resetButtonGradient}
                >
                  Reserve
                </button>
              </form>
